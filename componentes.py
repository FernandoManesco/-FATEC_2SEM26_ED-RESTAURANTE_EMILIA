...
class BotaoMenu(tk.Button):
    """
    Botao grande padronizado do menu principal.
    Herda de tk.Button -> um BotaoMenu E um Button, ja com o visual padrao.
    So variam: texto, cor de fundo e o comando (acao do clique).
    """

    def __init__(self, janela_pai, texto, cor, comando):
        # chama o construtor do Button (a classe pai) ja com TUDO configurado:
        # - o que VARIA vem dos parametros (texto, cor, comando)
        # - o que e FIXO fica escrito aqui direto (fonte, tamanho, cor do texto)
        super().__init__(
            janela_pai,
            text=texto,                 # <- varia
            bg=cor,                     # <- varia
            command=comando,            # <- varia
            font=("Arial", 14, "bold"), # fixo para todos
            fg="white",                 # fixo
            width=25,                   # fixo
            height=2                    # fixo
        )
